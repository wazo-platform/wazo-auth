...
NEW_ACL_TEMPLATE = '{% for tenant in visible_tenants %}dird.tenants.{{ tenant.name }}.#:{% endfor %}'
...
def _delete_acl_template(conn, old_acl_template_id):
    query = (acl_template_table
             .delete()
             .where(acl_template_table.c.id == old_acl_template_id))
    op.execute(query)
