...
def merge_external_auth_data_with_external_auth_config():
    ...
    sub_query = (
        sa.sql.select([external_auth_data_tbl.c.data])
        .where(external_auth_config_tbl.c.data_uuid == external_auth_data_tbl.c.uuid)
    )
    op.execute(external_auth_config_tbl.update().values(data=sub_query))
    op.execute(
        external_auth_config_tbl
        .delete()
        .where(external_auth_config_tbl.c.data == None)  # noqa
    )
...
def merge_external_auth_data_with_user_external_auth():
    ...
    sub_query = (
        sa.sql.select([external_auth_data_tbl.c.data])
        .where(
            user_external_auth_tbl.c.external_auth_data_uuid == external_auth_data_tbl.c.uuid
        )
    )
    op.execute(user_external_auth_tbl.update().values(data=sub_query))
    op.execute(
        user_external_auth_tbl
        .delete()
        .where(user_external_auth_tbl.c.data == None)  # noqa
    )
...
def unmerge_external_auth_data_with_external_auth_config():
    ...
    for external_auth in external_auths:
        data_uuid = op.get_bind().execute(
            external_auth_data_tbl
            .insert()
...
def unmerge_external_auth_data_with_user_external_auth():
    ...
    for external_auth in external_auths:
        ...
        op.execute(
            ...
            .where(user_external_auth_tbl.c.external_auth_type_uuid == external_auth.external_auth_type_uuid)
            .values(external_auth_data_uuid=data_uuid)
        )

    op.alter_column('auth_user_external_auth', 'external_auth_data_uuid', nullable=False)
