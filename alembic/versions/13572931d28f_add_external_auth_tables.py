...
def upgrade():
    ...
    op.create_table(
        ...
        Column('user_uuid', sa.String(38),
               sa.ForeignKey('auth_user.uuid', ondelete='CASCADE'), primary_key=True),
        Column('external_auth_type_uuid', sa.String(38),
               sa.ForeignKey('auth_external_auth_type.uuid', ondelete='CASCADE'), primary_key=True),
        Column('external_auth_data_uuid', sa.String(38),
               sa.ForeignKey('auth_external_auth_data.uuid', ondelete='CASCADE'), primary_key=True))

    op.create_unique_constraint(constraint_name, 'auth_user_external_auth',
