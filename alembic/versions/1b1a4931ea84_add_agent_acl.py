...
Revision ID: 1b1a4931ea84
Revises: 2c1a43889d04
...
ACL_TEMPLATES = [
    ...
    '{% if agent %}agentd.agents.by-id.{{ agent.id }}.login.create{% endif %}',
    '{% if agent %}agentd.agents.by-id.{{ agent.id }}.logoff.create{% endif %}',
    '{% if agent %}agentd.agents.by-number.{{ agent.number }}.read{% endif %}',
    '{% if agent %}agentd.agents.by-number.{{ agent.number }}.*.create{% endif %}',
]
