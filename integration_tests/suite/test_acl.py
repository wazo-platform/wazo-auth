...
MULTI_ACL_TEMPLATE = (
    '{% for resource in ("users", "lines") %}confd.{{ resource }}:{% endfor %}'
)


class TestACL(WazoAuthTestCase):
    @fixtures.http.policy(
        name='multple-acl-in-one-template', acl_templates=[MULTI_ACL_TEMPLATE]
    )
    ...
    def test_acl_template_loop_for(self, user, policy):
        ...
        assert_that(
            response, has_entry('acls', has_items('confd.users', 'confd.lines'))
        )
